...
def main():
    ...
    if gpu_mode:
        ...
        print(f"gpu mem before load xm is {gpu_memory}")
    
    xm = load_model(xm_name, device=device)
    print(f"load xm finish at {time.time()-init_time}")
    gpu_memory = get_gpu_memory_usage()
    print(f"gpu mem after load xm is {gpu_memory}")

    model = load_model(model_name, device=device)
    print(f"load diffusion model finish at {time.time()-init_time}")
    gpu_memory = get_gpu_memory_usage()
    print(f"gpu mem after load diffusion model is {gpu_memory}")

    diffusion = diffusion_from_config(load_config('diffusion'))
    print(f"load diffusion para model finish at {time.time()-init_time}")
    gpu_memory = get_gpu_memory_usage()
    print(f"gpu mem after load diffusion para is {gpu_memory}")
    ...
    print(f"load input image finish at {time.time()-init_time}")
    ...
    print(f"diffusion model start at {time.time()-init_time}")
    ...
    print(f"diffusion model end at {time.time()-init_time}")
    if timing_mode:
    ...
    print(f"rendering end at {time.time()-init_time}")
